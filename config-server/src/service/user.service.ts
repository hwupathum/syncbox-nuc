...
import {addNewUser, getUserByUsername, updateUserPasswordByUserId} from "../database/user_repository";
import { MySQLResponse } from "../model/mysql_response.model";
import { SeafileResponse } from "../model/seafile_response.model";
import { User } from "../model/user.model";
import {retrieveUserTokenFromConfigFile, retrieveUserTokenFromServer} from "../seafile_utils/retrieve_user_token";
...
export function createUser(input: Partial<User>, callback: Function) {
  ...
  getUserByUsername(username, (response: MySQLResponse) => {
    if (response.error) {
      ...
      retrieveUserTokenFromServer(username, password, (response: SeafileResponse) => {
          if (response.stdout) {
            let opt: any = JSON.parse(response.stdout);
            if (opt.non_field_errors) {
              log.error(`An error occurred ... ${opt.non_field_errors}`);
              callback(new CustomResponse(401, "Unable to register with the provided credentials", {}));
            } else if (opt.token) {
              ...
              createFile(`${user_directory}/seadrive.conf`, configFileTemplate(username, opt.token, username));
              ...
              hashPassword(password, (response: BcryptResponse) => {
                if (response.error) {
                  ...
                  getDirectoryHash(opt.token, response.hash, (password: string, response: SeafileResponse) => {
                      ...
                      addNewUser(username, password, `${user_directory}/data`, path_hash, (response: MySQLResponse) => {
                          if (response.error) {
                            log.error(`An error occurred ... ${response.error.message}`);
                            callback(new CustomResponse(500, "User saving failed. Try again", {}));
                          } else {
                            log.info(`Successfully logged in ...`);
                            callback(new CustomResponse(200, "", {username, token: opt.token}));
                          }
                        }
                      );
                    }
            ...
            log.error(`An error occurred ... ${response.error ? response.error.message : response.stderr}`);
            callback(new CustomResponse(500, "System failure. Try again", {}));
          }
        }
      );
    ...
    if (response.error) {
      ...
      retrieveUserTokenFromServer(username, password, (response: SeafileResponse) => {
          if (response.stdout) {
            let opt: any = JSON.parse(response.stdout);
            if (opt.non_field_errors) {
              comparePassword(password, user.password, (response: BcryptResponse) => {
                  if (response.error) {
                    log.error(`An error occurred ... ${response.error.message}`);
                    callback(new CustomResponse(500, "System failure. Try again", {}));
                  } else if (response.reply) {
                    // Server password changed but not updated the SyncBox
                    log.error(`${username} has changed the server password ...`);
                    callback(new CustomResponse(500, "Server password has changed", {}));
                  } else {
              ...
              comparePassword(password, user.password, (response: BcryptResponse) => {
                  if (response.error) {
                    log.error(`An error occurrred ... ${response.error.message}`);
                    callback(new CustomResponse(500, "System failure. Try again", {}));
                  } else if (!response.reply) {
                    log.info(`Server password of User: ${username} was changed ...`);
                    hashPassword(password, (response: BcryptResponse) => {
                      if (response.error) {
                        log.error(`An error occurrred ... ${response.error.message}`);
                        callback(new CustomResponse(500, "System failure. Try again", {}));
                        ...
                        updateUserPasswordByUserId(user.user_id, response.hash, (response: MySQLResponse) => {
                            if (response.error) {
                              log.error(`An error occurrred ... ${response.error.message}`);
                              callback(new CustomResponse(500, "System failure. Try again", {}));
                            }
                        ...
                        updateSeafileConfigurationFile(`${base_directory}/${username}/seadrive.conf`, opt.token);
                      }
                    });
                  }
            ...
            comparePassword(password, user.password, (response: BcryptResponse) => {
                if (response.error) {
                  ...
                  retrieveUserTokenFromConfigFile(`${base_directory}/${username}/seadrive.conf`, (error: string, token: string) => {
                      if (token) {
                        console.log(`${username} successfully logged in...`);
                        callback(new CustomResponse(200, "", { username, token }));
                      } else {
                  ...
                  callback(new CustomResponse(401, "Incorrect credentials", {}));
                }
              }
            );
          }
